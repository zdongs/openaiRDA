...
class DataAnalyzer:
    ...
    def quant_salary(self):
        def extract_numbers(salary_range):
            match = re.match(r'^(\d{1,3})k-(\d{1,3})k(?:·(\d+)薪)?$', str(salary_range))
            if match:
    ...
    def quant_work_exp(self):
        exp2inx = {'经验不限':0, '应届生':1, '1年以下':2, '1-3年':3, '3-5年':4, '5-10年':5, '10年以上':6}
        self.data['工作年限'] = self.data['工作年限'].map(exp2inx)
    ...
    def quant_scale(self, sortb = True):
        ...
        if sortb:
            ...
            self.data['企业规模'] = self.data['企业规模'].apply(extract_scale)
            return None
